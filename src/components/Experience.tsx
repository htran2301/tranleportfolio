      ...
      <div className="container-custom">
        <div className="text-center mb-16">
          ...
          <p className="section-subtitle mx-auto">
            A track record of research and academic excellence
          </p>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            ...
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className={`absolute w-4 h-4 rounded-full border-4 border-card top-6 ${
                  exp.current ? 'bg-accent' : 'bg-muted'
                } ${
                  index % 2 === 0 
                ...
                <Card className={`card-hover ${exp.current ? 'border-accent/50 shadow-soft' : 'border-border/50'}`}>
                  <CardContent className="p-6">
                    <div className={`flex flex-col gap-2 ${index % 2 === 0 ? 'md:items-end' : 'md:items-start'}`}>
                      {exp.current && (
                        <Badge className="bg-accent/10 text-accent border-0 w-fit">Current</Badge>
                      )}
